...
class SelfAttention:
    ...
    def store_qkv(self, qkv_home, qkv_write_buf, i):
        ...
        general_copy(q_home, None, q_new, None)
        general_copy(k_home, None, k_new, None)
    ...
    def store_value(self, value_home, value_write_buf, i):
        ...
        general_copy(h_home, None, h_new, None)
    ...
    def forward_preattn(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k, qkv_write_buf):
        n_head = self.config.n_head
        ...
        if i == 0:  # prefill
            ...
            q, new_k, new_v = self.compute.mha_gen_preattn(h, None, w_q,
                ...
                self.policy.compress_cache, self.policy.comp_cache_config)
            # Move q, k, v to cpu
            qkv_write_buf.store((q, new_k, new_v))
    
    def forward_attn(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k, qkv_read_buf, value_write_buf):
        n_head = self.config.n_head
        ...
        if i == 0:  # prefill
            ...
            cache_write_buf.store((new_k_cache, new_v_cache))
            value_write_buf.store(value)
    
    def forward_postattn(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
                cache_write_buf, i, k, value_read_buf):
        n_head = self.config.n_head
        ...
        if i == 0:  # prefill
            ...
            h = self.compute.mha_gen_postattn(h, None, w_q,
                b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                None, None, donate, self.policy.attn_sparsity,
                self.policy.compress_cache, self.policy.comp_cache_config, value)
